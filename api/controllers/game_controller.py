...
def obter_game(game_id: int, db: Session = Depends(get_db)):
    db_game = game_repository.get_game(db=db, game_id=game_id)
    if db_game is None:
...
def atualizar_game(game_id: int, game_update: schema.GameUpdate, db: Session = Depends(get_db)):
    db_game = game_repository.update_game(db=db, game_id=game_id, game_update=game_update)
    if db_game is None:
        raise HTTPException(status_code=404, detail="Jogo não encontrado")
    return db_game
...
def deletar_game(game_id: int, db: Session = Depends(get_db)):
    db_game = game_repository.delete_game(db=db, game_id=game_id)
    if db_game is None:
        raise HTTPException(status_code=404, detail="Jogo não encontrado")
    return None